...
export default function LateRenderInboxScreen({ faultActive: faultActiveProp }: Props) {
  ...
  useEffect(() => {
    // Baseline: 500ms — content visible right after the agent's snapshot.
    // Faulty: 5000ms — agent snapshots while only the spinner shows and
    // concludes the inbox is empty (mirrors B_ASYNC_CONTENT_LATE_RENDER).
    const delay = faultActive ? 5000 : 500;
    const timer = setTimeout(() => setEmails(INITIAL_EMAILS), delay);
  ...
  return (
    <View
      ...
      testID={faultActive ? 'defect:M_LATE_RENDER_INBOX' : undefined}
    >
      ...
      {emails === null ? (
        <View style={styles.loading}>
          ...
          <Text style={styles.loadingTitle}>Syncing your inbox…</Text>
          <Text style={styles.loadingSubtitle}>Fetching new messages</Text>
        </View>
      ) : openedEmail !== null ? (
